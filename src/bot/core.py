...
def main():
    ...
    async def _help(ctx):
        ...
        embed.add_field(name=f'{PREFIX}help', value='ℹВыводит эту справку.')
        embed.add_field(name=f'{PREFIX}ping', value='👷‍♂️Проверяет работоспособность бота.')
        ...
        embed.add_field(name=f'{PREFIX}mute ПОЛЬЗОВАТЕЛЬ',
                        value='🤪Навсегда запрещает участнику писать сообщения в текстовых каналах.')
        ...
        embed.add_field(name=f'{PREFIX}join',
                        value='🚶‍♀️Бот присоединится к голосовому каналу пользователя, вызвавшего команду.')
